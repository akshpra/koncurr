...
// *** NEW: News Feed Config ***
const NEWS_FEED_URL = 'https://economictimes.indiatimes.com/markets/rssfeeds/1977021501.cms'; // ET Markets RSS
const CORS_PROXY_URL = 'https://api.allorigins.win/get?url='; // Public CORS proxy
const MAX_NEWS_ITEMS = 50; // Number of news items to display
...
// *** NEW: News Feed Elements ***
const newsSeparatorElement = document.getElementById('news-separator');
const newsFeedSectionElement = document.getElementById('news-feed-section');
const newsLoaderElement = document.getElementById('news-loader');
const newsErrorElement = document.getElementById('news-error');
const newsFeedContentElement = document.getElementById('news-feed-content');
...
function calculateStats(values) { const validValues = values.filter(v => typeof v === 'number' && !isNaN(v)); const n = validValues.length; if (n <= 1) { return null; } const mean = validValues.reduce((a, b) => a + b, 0) / n; const variance = validValues.reduce((sqDiff, val) => sqDiff + Math.pow(val - mean, 2), 0) / n; if (isNaN(variance) || variance < 0 || isNaN(mean)) { console.warn("Could not calculate valid stats."); return null; } return { mean: mean, stdDev: Math.sqrt(variance) }; }
...
// --- *** NEW: News Feed Functions *** ---

/**
 * Fetches and parses the news feed using a CORS proxy.
 */
async function fetchNewsFeed() {
    if (!newsFeedSectionElement || !newsLoaderElement || !newsErrorElement || !newsFeedContentElement) return;

    console.log("Fetching news feed...");
    showBlockElement(newsSeparatorElement); // Show separator
    showBlockElement(newsFeedSectionElement); // Show section
    showElement(newsLoaderElement); // Show loader within section
    hideElement(newsErrorElement);
    hideElement(newsFeedContentElement);
    ...
    try {
        ...
        const rssContent = data.contents; // The actual RSS/XML content is usually in 'contents'
        ...
        if (parseError) {
            ...
            throw new Error("Failed to parse news feed data.");
        }

        // Extract items (adjust querySelectorAll based on RSS feed structure)
        const items = xmlDoc.querySelectorAll("item"); // Common tag for RSS items
        const newsItems = [];
        items.forEach((item, index) => {
            if (index >= MAX_NEWS_ITEMS) return; // Limit number of items
            ...
            let pubDate = null;
            if (pubDateStr) {
                try { pubDate = new Date(pubDateStr); } catch (e) { /* ignore invalid date */ }
            }

            newsItems.push({ title, link, pubDate });
        });

        displayNewsFeed(newsItems);

    } catch (error) {
        console.error("Error fetching or parsing news feed:", error);
        hideElement(newsLoaderElement);
        hideElement(newsFeedContentElement);
        setStatusMessage(newsErrorElement, `Failed to load news: ${error.message}`, true);
        showBlockElement(newsErrorElement); // Ensure error is visible
    } finally {
         // Ensure loader is hidden even if content doesn't display
         if (newsLoaderElement) hideElement(newsLoaderElement);
    }
}

/**
 * Displays the fetched news items in the UI.
 */
function displayNewsFeed(newsItems) {
    if (!newsFeedContentElement) return;
    newsFeedContentElement.innerHTML = ''; // Clear previous items

    if (!newsItems || newsItems.length === 0) {
        newsFeedContentElement.innerHTML = '<p class="p-4 text-sm text-slate-500 text-center">No news headlines found.</p>';
        showBlockElement(newsFeedContentElement);
        return;
    }
...
async function loadAllData() {
    ...
    hideElement(overlaySeparatorElement); hideElement(overlayChartSectionElement);

    try {
        // Fetch essential data first
        const [ratesResult, historyResult, allCurrenciesResult] = await Promise.allSettled([
            fetchLatestRates(), fetchHistoricalRates(), fetchAllCurrenciesList()
        ]);
        ...
        // Show and populate other sections
        populateConverterSelects(); convertCurrency();
        showBlockElement(converterSeparatorElement); showBlockElement(converterSectionElement);
        ...
        showBlockElement(downloadSeparatorElement); showBlockElement(downloaderSectionElement);

        populateOverlayCheckboxes(); updateOverlayChart(OVERLAY_CHART_DEFAULT_CURRENCIES);
        showBlockElement(overlaySeparatorElement); showBlockElement(overlayChartSectionElement);
        ...
        downloadButtonElement?.addEventListener('click', handleDownloadClick);

        // *** Fetch News Feed (runs after main content is displayed) ***
        fetchNewsFeed();


    } catch (error) {
        // Handle critical errors during initial load
        console.error("Error loading currency data:", error);
        hideElement(mainLoaderElement);
        // Ensure all sections remain hidden on critical error
        hideElement(currencyGridElement);
        hideElement(newsSeparatorElement); hideElement(newsFeedSectionElement);
        hideElement(converterSeparatorElement); hideElement(converterSectionElement);
        hideElement(downloadSeparatorElement); hideElement(downloaderSectionElement);
        hideElement(overlaySeparatorElement); hideElement(overlayChartSectionElement);
        setStatusMessage(mainErrorElement, `Error: ${error.message || 'Could not load currency data.'}`, true);
        showBlockElement(mainErrorElement); // Make sure main error is visible
    } finally {

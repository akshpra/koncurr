...
const OVERLAY_CHART_MAX_CURRENCIES = 4;
const OVERLAY_CHART_DEFAULT_CURRENCIES = ['USD', 'EUR'];
...
function getLightCurrencyColor(currencyCode) { const hexColor = getCurrencyColor(currencyCode); let r=0,g=0,b=0; if (hexColor.length == 7){ r=parseInt(hexColor.substring(1,3),16); g=parseInt(hexColor.substring(3,5),16); b=parseInt(hexColor.substring(5,7),16); } else if (hexColor.length == 4){ r=parseInt(hexColor.substring(1,2)+hexColor.substring(1,2),16); g=parseInt(hexColor.substring(2,3)+hexColor.substring(2,3),16); b=parseInt(hexColor.substring(3,4)+hexColor.substring(3,4),16); } return `rgba(${r}, ${g}, ${b}, 0.1)`; }
...
/**
 * Calculates the standard deviation and mean of an array of numbers.
 * Returns object { mean, stdDev } or null if calculation is not possible.
 */
function calculateStats(values) {
    ...
    if (n <= 1) {
        return null; // Calculation requires at least 2 points
    }

    const mean = validValues.reduce((a, b) => a + b, 0) / n;
    const variance = validValues.reduce((sqDiff, val) => sqDiff + Math.pow(val - mean, 2), 0) / n;

    if (isNaN(variance) || variance < 0 || isNaN(mean)) {
         console.warn("Could not calculate valid stats.");
         return null;
    }
